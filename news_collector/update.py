...
df['content'] = df['content'].str.replace('dedi', '')
df['title'] = df['title'].str.replace('dedi', '')
...
# rare_d = pd.Series(' '.join(temp_data_to_clean['merged']).split()).value_counts()[-17528:]
...
df.to_csv('news.csv',encoding='utf-8',sep=';', index=False)